...
X0 = [-1.0, -400.0, -9000.0, -60000.0, 0.5, 33.33]

# ── Espacio de búsqueda — centrado en la semilla, ±orden de magnitud ─────────
SPACE = [
    Real(-20.0,    -0.01,   name="a"),   # Kp AC      — negativo (pos_fb)
    Real(-5000.0,  -10.0,   name="b"),   # Ki AC      — negativo (pos_fb)
...
def _is_stable_point(a, b, c, d, e, f):
    Ltot, R, n_mmc, C_cap, Vc = 2.5e-3, 1.0, 3, 1.0, 150.0
    # AC y DC externo: realimentación positiva (controlador negativo compensa planta negativa)
    ok_ac  = _poles_stable(a, b, [1.0], [Ltot, R],             pos_fb=True)
